...
export default function SeatingPlannerPage() {
    ...
    const exportSeatingCSV = () => {
        const headers = ['Table Name', 'Table Type', 'Capacity', 'Guests Seated', 'Guest Names', 'Status'];
        const rows = eventTables.map(table => {
            const tableGuests = getTableGuests(table.id);
            const guestNames = tableGuests.map(g => `${g.firstName} ${g.lastName}`).join('; ');
            ...
            return [
                ...
                tableGuests.length,
                guestNames,
                isOver ? 'Over Capacity' : isFull ? 'Full' : 'Available'
            ];
        });
        ...
        link.href = url;
        link.download = `seating_chart_${event?.name || 'event'}_${new Date().toISOString().split('T')[0]}.csv`;
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => setFormData({ ...formData, tableType: e.target.value as 'round' | 'rectangular' | 'high_top' | 'vip' })}
                                className="input"
                            >
