...
import {
    ...
    Users, Trash2, X, Eye, Link2, Edit, Check
} from 'lucide-react';
...
export default function GuestsPage() {
    ...
    const getRsvpLink = (guest: Guest) => {
        const token = guest.rsvpToken || guest.id;
    ...
    const exportGuestsCSV = () => {
        const headers = ['First Name', 'Last Name', 'Email', 'Phone', 'Relationship', 'RSVP Status', 'VIP', 'Dietary Restrictions', 'Tags'];
    ...
    const filteredGuests = useMemo(() => {
        return guests.filter((guest) => {
            const matchesSearch =
                ...
                (guest.relationship?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false);

            const matchesFilter =
                filterStatus === 'all' ||
                (filterStatus === 'confirmed' && guest.globalRsvpStatus === 'yes') ||
                (filterStatus === 'pending' && guest.globalRsvpStatus === 'pending') ||
                (filterStatus === 'declined' && guest.globalRsvpStatus === 'no') ||
    ...
    const handleDeleteSelected = async () => {
        if (window.confirm(`Delete ${selectedGuests.size} selected guests?`)) {
...
function GuestFormModal({
    ...
    const [formData, setFormData] = useState<Partial<Guest>>(() => {
        if (guest) {
            // Ensure all optional string fields have empty string defaults
            return {
                ...guest,
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="flex items-center space-x-6">
                        <label className="flex items-center space-x-2">
                            <input
                                type="checkbox"
                                checked={formData.allowPlusOne || false}
